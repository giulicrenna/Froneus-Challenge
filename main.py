...
from src.gpt import (
    contexto_global,
    mensajes_presentacion,
    agregar_mensaje_contexto,
    obtener_mensajes_contexto,
    llamar_api_openai,
)
...
if not os.path.exists(os.path.join(os.getcwd(), "data", "embeddings")):
    ...
    pdf_path: str = os.path.join(
        os.getcwd(), "data", "pdf", "Juego de tronos - Cancion de hielo y fuego.pdf"
    )
...
with st.sidebar:
    st.image(
        os.path.join(os.getcwd(), "static", "froneus.png"), use_container_width=True
    )
    ...
    if st.button("Cargar usuario"):
        if usuario:
            if usuario not in contexto_global.contexto:
                contexto_global.contexto[usuario] = ContextoUsuario(
...
if prompt := st.chat_input(
    "Escribe tu pregunta:", disabled=True if usuario == "" else False
):
    ...
    respuesta: str = llamar_api_openai(
        usuario=usuario, pregunta=prompt, documentos=documentos
    )
